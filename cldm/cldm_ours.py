...
from ldm.models.diffusion.ddpm_ours import LatentDiffusion
...
class ControlNet(nn.Module):
    def __init__(
        ...
        self.input_hint_block_WSI = TimestepEmbedSequential(
            ...
            nn.SiLU(),
            zero_module(conv_nd(dims, 256, 123, 3, padding=1))
        )
        self.input_hint_block_LRST = TimestepEmbedSequential(
            conv_nd(dims, hint_channels, 16, 3, padding=1),
            nn.SiLU(),
            conv_nd(dims, 16, 16, 3, padding=1),
            nn.SiLU(),
            ...
            conv_nd(dims, 64, 256, 3, padding=1, stride=2),
            nn.SiLU(),
            zero_module(conv_nd(dims, 256, 100, 3, padding=1))
        )
        self.input_hint_block_GeneCodeMap = TimestepEmbedSequential(
            zero_module(conv_nd(dims, 1, 1, 3, padding=1))
        )
    ...
    def forward(self, x, hint_WSI,hint_LRST,hint_GeneCodeMap, timesteps, context, **kwargs):
        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False)
        emb = self.time_embed(t_emb)

        guided_hint_WSI = self.input_hint_block_WSI(hint_WSI, emb, context) # b, 123, 64, 64
        guided_hint_LRST = self.input_hint_block_LRST(hint_LRST, emb, context)# b, 100, 64, 64
        ## or just give original hint_GeneCodeMap
        guided_hint_GeneCodeMap=torch.nn.functional.interpolate(hint_GeneCodeMap,(64, 64))# b, 1, 64, 64


        guided_hint=torch.concat((guided_hint_WSI,guided_hint_LRST,guided_hint_GeneCodeMap),dim=1)



        ## downsample for reducing calculation of CL
        outs = []
        h = x.type(self.dtype)
...
class ControlLDM(LatentDiffusion):
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        ...
        control_Gene_index_map = control_Gene_index_map.to(memory_format=torch.contiguous_format).float()
    ...
    def apply_model(self, x_noisy, t, cond, *args, **kwargs):
        ...
        control = self.control_model(x=x_noisy, hint_WSI=torch.cat(cond['control_WSI'], 1),hint_LRST=torch.cat(cond['control_LRST'], 1),
                                     hint_GeneCodeMap=torch.cat(cond['control_Gene_index_map'], 1), timesteps=t, context=cond_txt)
